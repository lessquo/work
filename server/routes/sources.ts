  ...
  const rows = db
    .prepare(
      `SELECT
       ...
       FROM workflows w
       WHERE w.id IN (
         SELECT workflow_id FROM items WHERE workflow_id IS NOT NULL AND source_id = ?
         UNION
         SELECT s.workflow_id FROM sessions s
           LEFT JOIN items si ON si.id = s.item_id
          WHERE s.workflow_id IS NOT NULL AND (s.source_id = ? OR si.source_id = ?)
       )
       ORDER BY w.created_at DESC, w.id DESC`,
    )
    .all(id, id, id) as Array<{ id: number; name: string | null; created_at: string; updated_at: string; items: string; sessions: string }>;
