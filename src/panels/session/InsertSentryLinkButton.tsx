...
export function InsertSentryLinkButton({ onInsert }: { onInsert: (url: string) => void }) {
  const sourcesQuery = useQuery({ queryKey: ['sources'], queryFn: api.listSources });
  const sentrySources = (sourcesQuery.data ?? []).filter(s => s.type === 'sentry_issue');

  const itemsQueries = useQueries({
    queries: sentrySources.map(s => ({
    ...
    >
      ...
      <Combobox.Portal>
        <Combobox.Positioner sideOffset={4}>
          <Combobox.Popup
            className='popup flex max-h-128 w-lg flex-col overflow-hidden'
            aria-label='Insert Sentry link'
          >
            <div className='p-2'>
              <Combobox.Input
            ...
            <div className='min-h-0 flex-1 overflow-y-auto'>
              <Combobox.Empty className='px-3 py-2 text-xs text-gray-400'>
                {sentrySources.length === 0
                  ? 'No Sentry sources yet.'
                  : items.length === 0
                    ? '(no open issues)'
